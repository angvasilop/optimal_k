...
# choose models
variables<-list(c(1,2),c(1:3),c(1:4),c(1:5),c(1:10),
...
n <- 1000
nsim <- 1000
k <- c(2, 10, 50, 100, 250, 500, 750, 1000)
n_mod <- length(variables)
results <- array(NA, dim = c(nsim, length(k), n_mod))
mod <- list()
x[, (length(n_var) + 1):(length(n_var) + n_mod)] <- NA
  ...
  for(l in 1:length(k)){ # choose lth fold number to test
    ...
    for(i in 1:k[l]){ # outer cv loop
      for(m in 1:n_mod){print(c(j, l, i, m))
        best.lam[m] <- cv.glmnet(dat[fold != i, variables[[m]] + 1], dat[fold != i, 1], alpha = 1, folds = 5)$lambda.min # inner cv loop
        mod[[m]] <- glmnet(dat[fold != i, variables[[m]] + 1], dat[fold != i, 1], alpha = 1, lambda = best.lam[m])
        dat[fold == i, ncol(dat[, 1:101]) + m] <- predict(mod[[m]], dat[fold == i, variables[[m]] + 1])
      }
    }
    predictions <- (ncol(dat) + 1 - n_mod):ncol(dat)
    for(p in predictions){
      results[j, l, which(predictions == p)] <- mean((dat[, 1] - dat[, p])^2)
    }
  }
}
