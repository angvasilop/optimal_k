...
with constants $B$ to $E$ specific to each $n$ and constants $B$ and $E$ set to zero for linear regression. We then draw a line $L$ through $(k_1, \hat{A}_1)$ and $(k_n, \hat{A}_n)$ and maximize the perpendicular distance $d$ of each point on $\hat{A}$ from $L$, so that
$$k_\mathrm{optimal}^* = {\arg\max}_k{d[\hat{A}(k)-L]}.$$
