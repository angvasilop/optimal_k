...
To estimate $k_\mathrm{optimal}*$ for each $n$, we first fit to the dataset $(k,A)$ for each sample size $n$ a non-linear least squares model of the form
$$\hat{A}(k) = Bk + C + Dk^{-1} + Ek^{-2}$$
with constants $B$ to $E$ specific to each $n$ and constants $B$ and $E$ set to zero for linear regression. We then draw a line $L$ through $(k_1, \hat{A}_1)$ and $(k_n, \hat{A}_n)$ and maximize the perpendicular distance $d$ of each point on $\hat{A}$ from $L$, so that $k_\mathrm{optimal}^* = {\arg\max}_k{d[\hat{A}(k)-L]}$.
...
```{r echo=FALSE, fig.height = 8, fig.cap = "Count of true linear regression model selection by the lowest-error criterion vs. fold number for various sample sizes."}
figure7
```

```{r echo=FALSE, fig.cap = "Count of true LASSO regression model selection by the lowest-error criterion vs. fold number for various sample sizes."}
figure8
```
